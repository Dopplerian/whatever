...
public class NPCControllerAR : MonoBehaviour
{
    ...
    public void SpawnOnHorizontalPlane()
    {
        ...
        if (isEnemy)
        {
            ...
            logger.Log($"NPC Appear: Enemy spawned at {randomPoint}");
        }
        else
        {
            NPC = Instantiate(PrefabAlly, randomPoint, Quaternion.identity);
            logger.Log($"NPC Appear: Ally spawned at {randomPoint}");
        }
    ...
    private IEnumerator DestroyNPCAfterTime(GameObject npc, float delay)
    {
        ...
        if (npc != null)
        {
            logger.Log($"NPC Disappear: NPC destroyed: {npc.name}");
            Destroy(npc);
        }
    }
}
